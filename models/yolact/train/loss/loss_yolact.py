...
class YOLACTLoss(object):
  ...
  def __call__(self, pred, label, num_classes):
    ...
    loc_loss = self._loss_location(
        pred_offset, box_targets, positiveness) * self._loss_weight_box
    conf_loss = self._loss_class(
        pred_cls, cls_targets, num_classes, positiveness) * self._loss_weight_cls
    ...
    seg_loss = self._loss_semantic_segmentation(
        seg, masks, classes, num_obj) * self._loss_weight_seg
  ...
  def _loss_location(self, pred_offset, gt_offset, positiveness):
    ...
    loss_loc = tf.reduce_sum(smoothl1loss(
        gt_offset, pred_offset)) / tf.cast(num_pos, tf.float32)
  ...
  def _loss_class(self, pred_cls, gt_cls, num_cls, positiveness):
    ...
    neg_minus_log_class0_sort = tf.argsort(
        neg_minus_log_class0, direction="DESCENDING")
    ...
    target_logits = tf.concat(
        [pos_pred_cls, neg_pred_cls_for_loss], axis=0)
    target_labels = tf.cast(
        tf.concat([pos_gt, neg_gt_for_loss], axis=0), tf.int64)
    ...
    loss_conf = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(
        target_labels, target_logits)) / tf.cast(num_pos, tf.float32)
